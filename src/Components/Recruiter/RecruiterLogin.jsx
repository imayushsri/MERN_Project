...
const { register, handleSubmit, formState: { errors } } = useForm({
  ...
  const handleData = async(data) =>{
    const payload = {
      ...
      // API Call
    const response = await axios.post('http://localhost:8000/api/recruiter-login', payload, {
        headers:{
      ...
      if(response.data.code == 200){
        ...
        localStorage.setItem('userType', JSON.stringify('recruiter'));   //use in dynamic navbar
    ...
    return (
        <>
            <div className="form">
                ...
                <h2 className='form_h2'>Recruiter Sign In</h2>
